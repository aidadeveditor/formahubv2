 ...
 "sections": [
  {"titre": "Ce qu'un outil d'analytics compte vraiment",
   "paras": [
     ...
     "Une <strong>session</strong> est une suite d'événements provenant d'un même navigateur, close par une période d'inactivité — trente minutes dans la plupart des outils. Elle se termine donc au bout de trente minutes sans activité, même si la personne revient ensuite, et une même visite humaine interrompue par un déjeuner produit deux sessions. Universal Analytics, aujourd'hui hors service, la clôturait aussi à minuit ou lors d'un changement de source ; les outils actuels ne le font plus, seule l'inactivité découpe les sessions. La session n'est pas une unité naturelle : c'est une convention de découpage, et il faut savoir laquelle votre outil applique.",
...
"""},
     ...
     {"type": "pitfall", "titre": "poser des UTM sur des liens internes",
      "paras": [
        "L'idée paraît bonne : marquer une bannière de la page d'accueil avec des UTM pour savoir combien de personnes cliquent dessus. Elle est destructrice. L'arrivée d'un paramètre de campagne <strong>réattribue la source</strong> de la session en cours. Le visiteur venu de la publicité qui clique sur votre bannière interne cesse d'être attribué à la publicité et devient attribué à la bannière. Sur les outils historiques, cela ouvrait en plus une nouvelle session ; ce n'est plus le cas aujourd'hui, où seule l'inactivité découpe les sessions — mais la destruction de l'attribution, elle, demeure.",
  ...
  {"titre": "L'attribution : à quel canal revient la conversion ?",
   "paras": [
     ...
     "Le <strong>premier clic</strong> fait l'erreur symétrique, en attribuant tout à la découverte et rien à ce qui a conclu. Les modèles <strong>linéaire</strong> et à <strong>dépréciation temporelle</strong> répartissent la valeur entre les points de contact, également pour le premier, avec un poids croissant vers la fin pour le second — utiles à comprendre, mais retirés de Google Analytics en 2023 : l'outil ne propose plus que le dernier clic et le modèle piloté par les données, ce dernier étant désormais le défaut. Les modèles dits <strong>pilotés par les données</strong> estiment statistiquement la contribution de chaque point de contact ; ils demandent un volume important, et leur calcul n'est généralement pas inspectable, ce qui rend délicat de défendre une décision budgétaire devant quelqu'un qui la conteste.",
...
<p><strong>4. Le cas grave : des UTM sur un lien interne.</strong> C'est la seule erreur de la liste qui détruit des données existantes plutôt que de mal les ranger. Chaque clic sur cette bannière réattribue le visiteur à <code>site / interne</code>. Résultat : les campagnes d'acquisition perdent les conversions des visiteurs qui ont cliqué sur la bannière, et l'on peut couper un budget publicitaire sur la foi de ce constat. Correction : retirer les paramètres et mesurer le clic par un événement dédié.</p>
  ...
  "questions": [
   ...
   {"id":"q3","question":"Peut-on poser des paramètres UTM sur la bannière de sa propre page d'accueil ?",
    "choices":[{"key":"a","text":"Oui, c'est le moyen normal de mesurer les clics internes"},
               {"key":"b","text":"Non : cela réattribue la source de la session en cours, détruisant la vraie origine du visiteur"},
